"""
Contains a custom class for creating Markov Chain Generators.
...
class Markov:
    ...
    def train(self) -> None:
        ...
        for i in range(len(self.words)):
            ...
            if word not in self.chain:
                self.chain[word] = []
            self.chain[word].append(self.words[i + 1]) if i < len(self.words) - 1 else None

    def generate(self, seed: typing.Optional[str] = None, *, word_count: int) -> typing.Tuple:
        """
        ...
        if not self.chain:
            raise NotTrainedError("Before generating anything, the Markov chain must be trained")
    ...
    def new_generator(self, seed: typing.Optional[str] = None):
        """
        :param seed: The starting seed of the chain generator. Should be either None or an element in the dataset.
         If None, uses a random element in the dataset.
